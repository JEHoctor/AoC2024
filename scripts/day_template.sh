...
DAY_NUMBER=$($PYTHON_INTERPRETER -c "import inflect; p = inflect.engine(); print(p.number_to_words('$DAY_NUMERAL'))")
DAY_NUMBER_WITH_UNDERSCORES=$(tr ' -' '_' <<< $DAY_NUMBER)

NEW_FILE=$CMD_DIRECTORY/$DAY_NUMERAL.go
cp $TEMPLATE_PATH $NEW_FILE
sed -i "s/{{DAY_NUMBER}}/$DAY_NUMBER/g" $NEW_FILE
sed -i "s/{{DAY_NUMBER_WITH_UNDERSCORES}}/$DAY_NUMBER_WITH_UNDERSCORES/g" $NEW_FILE
